...
namespace Lockstep
{
    ...
    {
        const int m_ServerSetupIntervalSec = 2;
        const int m_ClientConnectIntervalSec = 2;
        ...
        {
            StartCoroutine(SendMessageUnderArtificialConditions(message, sendMode));
        }

        IEnumerator SendMessageUnderArtificialConditions(Message message, SendMode sendMode)
        {
            // Artifical latency
        ...
        IEnumerator SetupServer(int port)
        {
            while(!TrySetupServer(port))
            {
                ...
                yield return new WaitForSecondsRealtime(m_ServerSetupIntervalSec);
            }
        }
        ...
        IEnumerator ConnectClient(string peerAddress, int peerPort)
        {
            while (true)
            {
                ...
                yield return new WaitForSecondsRealtime(m_ClientConnectIntervalSec);
            }
        }
